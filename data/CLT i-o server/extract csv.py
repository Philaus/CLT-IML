import os
from pathlib import Path
import pandas as pd
import matplotlib.pyplot as plt


def extract_and_plot_csv_data():
    # 1. Define the target columns to extract.
    target_columns = ["Fr0", "Fdelta", "FA0", "Flamuta"]
    extracted_data = []

    # Get the current working directory.
    current_dir = Path(".")

    print("开始遍历文件夹并提取数据...")

    # 2. Iterate over immediate subfolders of the current directory.
    for subdir in current_dir.iterdir():
        if subdir.is_dir():
            if subdir.name == "不好":
                continue
            # Find all .csv files in the subfolder.
            for csv_file in subdir.glob("*.csv"):
                try:
                    # Read the CSV file.
                    df = pd.read_csv(csv_file)

                    # Check that all required columns are present.
                    if all(col in df.columns for col in target_columns):
                        # Extract the first row (index 0).
                        row_data = df[target_columns].iloc[0].to_dict()
                        # Record file and folder names for optional provenance tracing.
                        row_data["source_file"] = f"{subdir.name}/{csv_file.name}"
                        extracted_data.append(row_data)
                except Exception as e:
                    print(f"读取文件 {csv_file} 时出错: {e}")

    # 3. Check whether any data were extracted successfully.
    if not extracted_data:
        print("未找到包含指定列的 CSV 数据，请检查列名或文件路径。")
        return

    # Convert the data to a DataFrame for easier processing.
    result_df = pd.DataFrame(extracted_data)
    print(f"成功提取了 {len(result_df)} 个 CSV 文件的数据。")

    # 4. Create the scatter plots.
    # Create a 2x2 subplot canvas.
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    axes = axes.flatten()  # Flatten the 2x2 matrix for iteration.

    # Set the optional plot style.
    (
        plt.style.use("seaborn-v0_8-whitegrid")
        if "seaborn-v0_8-whitegrid" in plt.style.available
        else plt.style.use("default")
    )

    # Draw a separate scatter plot for each variable.
    for i, col in enumerate(target_columns):
        ax = axes[i]
        # Use the file index on the x-axis and the variable value on the y-axis.
        ax.scatter(
            result_df.index,
            result_df[col],
            color="royalblue",
            s=50,
            alpha=0.8,
            edgecolors="none",
        )

        # Set the title and labels.
        ax.set_title(f"Scatter Plot of {col}", fontsize=14, fontweight="bold")
        ax.set_xlabel("File Index", fontsize=11)
        ax.set_ylabel(f"Value ({col})", fontsize=11)
        ax.grid(True, linestyle="--", alpha=0.6)

        # For small datasets, file names can optionally be used as x-axis ticks.
        if len(result_df) <= 15:
            ax.set_xticks(result_df.index)
            ax.set_xticklabels(
                result_df["source_file"], rotation=45, ha="right", fontsize=8
            )

    # Adjust the layout to prevent overlap.
    plt.tight_layout()

    # Optionally save the figure in the current directory.
    plt.savefig("csv_data_scatter_plots.png", dpi=300)
    print("散点图已绘制完成，并保存为 'csv_data_scatter_plots.png'")

    # Display the plot.
    plt.show()


if __name__ == "__main__":
    extract_and_plot_csv_data()
